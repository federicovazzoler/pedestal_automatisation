...
// Author: Rene Brun
void makeNTuple_NoCrystalInfo(string treePATH, string outFileNAME_today, string runSummary, string fillSummary, string todayFileList, Int_t numRunAnalyze, Int_t k, Int_t files_to_process){
  ...
  for(int j=0; j<L; j++){
    cout << mfill->GetAt(j) << "   " << runt->GetAt(j) << endl;
...
////////////////////////////////////////
  ...
  while (fgets(line2,1024,fpl)) {
    ...
    const int max_number_char = 4000;
    char line[max_number_char];
    char *dummyLinefgets;
    
    // Check if the file is corrupted (third coloumn check)
    //FILE *fp_test_corrupted;
    //char line_test[400];
    //float laser_check_corrupted = -99.0;
    //float test_variable = 0;
    //int row_corrupted = 0;
    //char *dummyLinefgets_test;
    //int row_analysed = 0;
/* 
    // First two lines doesn't matter
    fp_test_corrupted = fopen(line2,"r");
    dummyLinefgets_test = fgets(line_test,400,fp_test_corrupted);
    dummyLinefgets_test = fgets(line_test,400,fp_test_corrupted);
    // Read first 100 rows and if laser_check_corrupted = -99 for more than 50 times the file is corrupted
    while (fgets(line_test,400,fp_test_corrupted) && row_analysed < 100) {
        sscanf(&line_test[0],"%f %*d %f",&test_variable,&laser_check_corrupted);
        //cout << "file " << line2 << " laser_check_corrupted = " << laser_check_corrupted << " test = " << test_variable << endl;
        row_analysed++;
        if(laser_check_corrupted < -90.0){
            row_corrupted++;
        }
    }
    fclose(fp_test_corrupted);
    // Decide if the file is good
    if(row_corrupted >= 50){
        cout << "[WARNING]: File " << line2 << " is heavily corrupted (more than 50 rows)! Let's skip it." << endl;    
        continue;
    }
*/
    // FILE OPENING: end
...
//exit(-1);
    ...
    if(processed_files%1000==0 && processed_files!=0) cout << "[INFO]: Completed " << double(processed_files)/double(files_to_process)*100 << "%" << endl;
...
//
      ...
      dummyLinefgets = fgets(line,max_number_char,fp);
      int idtime_;
    ...
    //std::cout<< "Run="<<run_<<" seq_id="<<seq_id<<" fed="<<fed_num<<" las_id="<<las_id<<std::endl;
      ...
      //int laser_control = 0; //bvariabile di controllo, terza colonna dei file di testo

      int elch=0; 
      while (fgets(line,max_number_char,fp)) {
        if(i<nchan[idfed]){
        ...
        //
            ...
            if(FED > 609 && FED < 646) { // EB
              ...
              if(iChannel < 0 || iChannel > 61199) {
  		        cout << "[ERROR]: EB strange channel " << iChannel << " for FED " << fed << " local ch " << elch << " file " << line2 << endl;
                cout << "       : Line " << line << endl;
                continue;
                //exit(-1);
              } 
              ...
              if(iChannel < 0 || iChannel > 14647) {
  		        cout << "[ERROR]: EE strange channel " << iChannel << " for FED " << fed << " local ch " << elch << " file " << line2 << endl;
                cout << "       : Line " << line << endl;
                continue;
                //exit(-1);
              }
            ...
            // MODIFICATO 13.09
            //cout << "[ERROR]: File " << line2 << endl;
            //std::cout<<"why this line after "<<i<<" channels is crazy ? "<<line<<std::endl; 
            //cout << "" << endl;
            
            rowCorruptedFound = true;

            // MODIFICA IMPORTANTE OCCHIO
            i++;
      ...
      if (rowCorruptedFound){
        cout << "[WARNING]: File " << line2 << " has one (or more) corrupted lines (p1 = -99)" << endl;
      }
      fclose(fp);
    }
  }
